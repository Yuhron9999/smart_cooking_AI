export class GoogleMapsService {
  ...
  async findNearbyStores(location: { lat: number; lng: number }, ingredients: string[]) {
    try {
      const response = await fetch(`/api/maps/nearby-stores?lat=${location.lat}&lng=${location.lng}&radius=2000&type=supermarket`);
  ...
  async getRegionalSuggestions(location: { lat: number; lng: number }) {
    try {
      const response = await fetch(`/api/maps/regional-suggestions?lat=${location.lat}&lng=${location.lng}`);
      const data = await response.json();
...
window.getDirections = (placeId: string) => {
  window.open(`https://www.google.com/maps/dir/?api=1&destination_place_id=${placeId}`, '_blank');
};